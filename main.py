...
def getWiFInfo(wifi_name):
    ...
    cmd_output = subprocess.run(f"netsh wlan show profiles \"{wifi_name}\" key=clear",shell=True,stdout=subprocess.PIPE)
    formatted_output = cmd_output.stdout.decode()
    
    security_key = re.search("Security key           : (.*)",formatted_output).group(0).replace("\r","")
    security_key = security_key.split(": ")[1] # split and store only security status
